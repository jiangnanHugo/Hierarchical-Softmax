...
class HierarchicalSoftmax():
    ...
    def __init__(self, tree, size, mb_size):
        ...
        for layer in tree:
            for i in layer:
                if isinstance(i, TreeNode): # middle units
                    self.node_count += 1
                    self.nodes.append(i)
                    self.node_dict[i.index] = i

                if isinstance(i, ResultNode): # leaf units
                    self.label_count += 1
                ...
                if isinstance(i, TreeNode):
                    try:
                        if not isinstance(i.left.index, str):
                            ...
                            tree_matrix_val[i.index][0] = i.index
                        ...
                        if not isinstance(i.right.index, str):
                            ...
                            tree_matrix_val[i.index][1] = i.index
        ...
        wp_val=np.asarray(self.rng.uniform(low=-np.sqrt(6. / (size + 2)), high=np.sqrt(6. / (size + 2)),
                                           size=(len(self.nodes) + 1, size)),dtype=theano.config.floatX)
        self.wp_matrix = theano.shared(value=wp_val,name='V_soft',borrow=True)
        #Parameter_matrix_b
        #self.bp_matrix = theano.shared(value=np.zeros((len(self.nodes), 2),dtype=theano.config.floatX),name='b_soft',borrow=True)
        ...
        node=T.sum(wp*self.x.dimshuffle(0,'x',1),axis=2)
        # activation=T.batched_dot(W,x)

        log_sig = T.log(T.nnet.sigmoid(node*choices))*mask
        ...
        self.train_f=theano.function(inputs=[self.x,self.y],
                                     outputs=self.cost,
                                     updates=updates)
        ...
        self.input_vectors_m = T.fmatrix()
