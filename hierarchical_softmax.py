...
class HierarchicalSoftmax():
    ...
    def __init__(self, tree, size, mb_size):
        ...
        wp_val=np.asarray(self.rng.uniform(low=-np.sqrt(6. / (size + 2)), high=np.sqrt(6. / (size + 2)),size=(len(self.nodes) + 1, size)),dtype=theano.config.floatX)
        ...
        for i in range(self.mb_size):
            ebin.append(np.arange(self.max_route_len))
        odx = T.as_tensor_variable(np.asarray(ebin))
        iv = self.x.reshape((self.x.shape[0], 1, self.x.shape[1]))

        #print("block sparse testing...")
        gb = T.nnet.blocksparse.SparseBlockGemv()
